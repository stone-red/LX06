...
namespace DAL
{
    public class Comment
    {
        ...
        public int GetCount()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * count (1) form Comment";
        ...
        public int GetWorkCount(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * count (1) form Comment where workId=@workId";
        ...
        public int Add(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into Comment(commentID<workId,userName,CommentContent,CommentTime)" +
                    "values(@commentIDm@workId,@workId,@userName,@CommentContent,@CommentTime);";
                sql += "SELECT @@IDENTITY";
        ...
        public int Update(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update Comment set CommentContent=@Commtent,CommentTime=@CommentContent where commentID=@commentID";
